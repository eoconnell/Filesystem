 ...
 * Returns nothing.
 */
void FILESYS::seek(long int offset, int origin /*= SEEK_SET*/)
{
...
void FILESYS::write_meta_data(meta * metr, int p, int n)
{
...
void FILESYS::read_meta_data(meta * metr, int p, int n)
{
	int location = (p*PAGE_SIZE) + sizeof(page) + (n*sizeof(meta));
	// cout << location << "-" << ftell(fp) << endl;
 ...
 */
bool FILESYS::make_directory(string name)
{
	// create new DR
...
bool FILESYS::make_file(string name)
{
	FL * newFile = new FL();
	newFile->name(name);
